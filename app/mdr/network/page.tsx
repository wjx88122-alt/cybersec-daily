...
function UsageBar({ value, label }: { value: number; label: string }) {
  ...
  return (
    <div className="flex-1">
      <div className="flex justify-between text-[10px] text-[#484f58] mb-0.5">
...
function ClientCard({ client, selected, onClick, alertCount }: { client: Client; selected: boolean; onClick: () => void; alertCount: number }) {
  return (
    <button onClick={onClick}
      className={`w-full text-left glass rounded-xl p-4 transition-all ${selected ? "border-[#e5ff00]/30 glow-accent" : "hover:border-white/[0.12]"}`}>
      ...
      <div className="flex items-center justify-between mt-2">
        <div className="flex items-center gap-1">
          ...
          <span className="text-[10px] text-[#484f58]">健康分</span>
        </div>
        {alertCount > 0 && (
...
function DeviceCard({ device }: { device: NetworkDevice }) {
  return (
    <div className="glass rounded-xl p-3 hover:border-white/[0.12] transition-all">
      <div className="flex items-center gap-2 mb-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[10px] text-[#484f58]">{device.ip} · {DEVICE_TYPE_LABELS[device.type]}</div>
        </div>
        <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/[0.06] text-[#6e7681]">{device.zone}</span>
      </div>
    ...
    <div className="min-h-screen" style={{ background: "var(--bg)" }}>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-xl font-bold text-white mt-1">客户网络运维中心</h1>
          <p className="text-xs text-[#484f58] mt-1">多客户网络设备监控 · 告警管理 · 运维工单</p>
        </div>

        {/* Global stats */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {[
            { icon: "🏢", label: "托管客户", value: MOCK_CLIENTS.length, color: "text-[#e5ff00]" },
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="lg:w-64 shrink-0 space-y-2">
            <div className="text-xs text-[#8b949e] font-medium mb-2">客户列表</div>
            {MOCK_CLIENTS.map((c) => (
          ...
          <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center justify-between flex-wrap gap-2">
                ...
                <div className="flex items-center gap-4">
                  <div className="text-center">
                    ...
                    <div className="text-[10px] text-[#484f58]">健康评分</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            {tab === "overview" && (
              <div className="space-y-4">
                ...
                <div className="glass rounded-xl p-4">
                  <div className="text-xs text-[#8b949e] font-medium mb-3">设备状态分布</div>
                    ...
                    {(["online", "warning", "critical", "offline"] as DeviceStatus[]).map((s) => (
                      <div key={s} className="text-center">
                        <div className="flex items-center justify-center gap-1.5 mb-1">
                          ...
                          <span className="text-[10px] text-[#8b949e]">{{ online: "正常", warning: "警告", critical: "严重", offline: "离线" }[s]}</span>
                        </div>
                ...
                <div className="glass rounded-xl p-4">
                  <div className="text-xs text-[#8b949e] font-medium mb-3">🗺️ 网络区域拓扑</div>
                      ...
                      return (
                        <div key={zone} className="glass rounded-lg p-3 min-w-[120px]">
                          <div className="flex items-center gap-1.5 mb-1">
                            ...
                            <span className="text-xs font-medium text-white">{zone}</span>
                          </div>
                          <div className="text-[10px] text-[#484f58]">{zDevices.length} 台设备</div>
                ...
                {alerts.filter((a) => !a.acknowledged).length > 0 && (
                  <div>
                    <div className="text-xs text-[#8b949e] font-medium mb-2">🔴 未确认告警</div>
                      ...
                      {alerts.filter((a) => !a.acknowledged).slice(0, 3).map((a) => (
                        <div key={a.id} className="glass rounded-xl p-3 flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-[10px] text-[#484f58]">{a.deviceName} · {timeAgo(a.timestamp)}</div>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {tab === "devices" && (
              <div>
                <div className="flex gap-1 mb-3 flex-wrap">
                  {(["all", "online", "warning", "critical", "offline"] as const).map((s) => (
                    <button key={s} onClick={() => setStatusFilter(s)}
                      className={`px-2.5 py-1 text-[11px] rounded-lg transition-all ${statusFilter === s ? "bg-white/[0.08] text-white border border-white/[0.1]" : "text-[#8b949e] hover:bg-white/[0.05]"}`}>
                ...
                {filteredDevices.length === 0 && <div className="text-xs text-[#484f58] text-center py-8">无匹配设备</div>}
              </div>
            )}
            ...
            {tab === "alerts" && (
              <div className="space-y-2">
                <div className="text-xs text-[#8b949e] mb-2">{alerts.length} 条告警</div>
                {alerts.map((a) => (
                  <div key={a.id} className={`glass rounded-xl p-3 flex items-center gap-3 ${a.acknowledged ? "opacity-50" : ""}`}>
                    ...
                    <span className={`text-[10px] px-1.5 py-0.5 rounded ${a.acknowledged ? "bg-green-500/10 text-green-400" : "bg-white/[0.06] text-[#8b949e]"}`}>
                      {a.acknowledged ? "已确认" : "待确认"}
                    </span>
                  </div>
                ))}
                {alerts.length === 0 && <div className="text-xs text-[#484f58] text-center py-8">暂无告警 ✅</div>}
              </div>
            )}
            ...
            {tab === "ops" && (
              <div className="space-y-2">
                <div className="text-xs text-[#8b949e] mb-2">{tickets.length} 条运维工单</div>
                {tickets.map((t) => (
                  <div key={t.id} className="glass rounded-xl p-4 hover:border-white/[0.12] transition-all">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <span className="text-[10px] text-[#484f58] font-mono">{t.id}</span>
                      ...
                      <span className="text-[10px] text-[#484f58]">{opsTypeIcon[t.type]} {opsTypeLabel[t.type]}</span>
                    </div>
                ...
                {tickets.length === 0 && <div className="text-xs text-[#484f58] text-center py-8">暂无工单</div>}
              </div>
            )}
          </div>
        </div>
