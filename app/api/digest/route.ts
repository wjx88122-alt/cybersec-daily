...
export async function GET(req: NextRequest) {
  ...
  if (auth === `Bearer ${process.env.CRON_SECRET}`) {
    ...
    if (!feedA) return NextResponse.json({ error: "No feed data, run cron first" }, { status: 400 });
  ...
  try {
    ...
    if (!digest) return NextResponse.json({ error: "No digest yet, run cron first" }, { status: 404 });
