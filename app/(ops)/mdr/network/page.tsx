...
function UsageBar({ value, label }: { value: number; label: string }) {
  const color = mdrLoadBarClass(value / 100, { dangerAt: 0.85, warningAt: 0.65 });
  return (
...
function ClientCard({ client, selected, onClick, alertCount }: { client: Client; selected: boolean; onClick: () => void; alertCount: number }) {
  return (
    <button onClick={onClick}
      className={`w-full text-left glass rounded-xl p-4 transition-all ${selected ? "border-blue-200 glow-accent" : "hover:border-black/[0.1]"}`}>
      ...
      <div className="flex items-center justify-between mt-2">
        <div className="flex items-center gap-1">
          <span className={`text-lg font-bold ${mdrHealthScoreToneClass(client.networkScore)}`}>{client.networkScore}</span>
          <span className="text-[10px] text-[#94a3b8]">健康分</span>
        </div>
        {alertCount > 0 && (
          <span className={`text-[10px] px-2 py-0.5 rounded-full border ${mdrSeverityBadgeClass("critical")}`}>{alertCount} 告警</span>
        )}
      </div>
    </button>
  );
              ...
              <div className="flex items-center justify-between flex-wrap gap-2">
                ...
                <div className="flex items-center gap-4">
                  <div className="text-center">
                    <div className={`text-2xl font-bold ${mdrHealthScoreToneClass(client.networkScore)}`}>{client.networkScore}</div>
                    ...
                    <div className="space-y-2">
                      {alerts.filter((a) => !a.acknowledged).slice(0, 3).map((a) => (
                        <div key={a.id} className="glass rounded-xl p-3 flex items-center gap-3">
                          <span className={`text-[10px] px-2 py-0.5 rounded-full border shrink-0 ${mdrSeverityBadgeClass(a.severity)}`}>{a.severity}</span>
                          <div className="flex-1 min-w-0">
